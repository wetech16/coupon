...
const CommentForm = (props) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    console.log(`this is body ${(screamId, body)}`);
  ...
  const commentFormMarkup = authenticated ? (
    <Grid item sm={12} style={{ textAlign: "center" }}>
      <form onSubmit={handleSubmit}>
        ...
        >
          Submit
          {loading && (
            <CircularProgress className={classes.button} size={10} />
          )}
        </Button>
      </form>
    ...
    <Typography align="center" color="secondary">
      ...
      or <Link to="/signup">Sign up </Link> and join the conversation
    </Typography>
  );
